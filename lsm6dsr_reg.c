...
int32_t lsm6dsr_xl_flag_data_ready_get(const stmdev_ctx_t *ctx,
                                       uint8_t *val)
{
  lsm6dsr_status_reg_t status_reg = {0};
  int32_t ret = {0};
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_write_reg(ctx, LSM6DSR_X_OFS_USR, (uint8_t *)val, 1);
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_read_reg(ctx, LSM6DSR_X_OFS_USR, (uint8_t *)val, 1);
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_write_reg(ctx, LSM6DSR_Y_OFS_USR, (uint8_t *)val, 1);
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_read_reg(ctx, LSM6DSR_Y_OFS_USR, (uint8_t *)val, 1);
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_write_reg(ctx, LSM6DSR_Z_OFS_USR, (uint8_t *)val, 1);
                                    ...
                                    int8_t *val)
{
  ...
  ret = lsm6dsr_read_reg(ctx, LSM6DSR_Z_OFS_USR, (uint8_t *)val, 1);
...
int32_t lsm6dsr_fifo_out_raw_get(const stmdev_ctx_t *ctx, uint8_t *val)
{
  ...
  ret = lsm6dsr_read_reg(ctx, LSM6DSR_FIFO_DATA_OUT_X_L, val, 6);
...
{
  lsm6dsr_emb_func_src_t emb_func_src = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_internal_freq_fine_t internal_freq_fine = {0};
  int32_t ret = {0};
                            ...
                            uint8_t *buf, uint8_t len)
{
  ...
  lsm6dsr_page_sel_t page_sel = {0};
  lsm6dsr_page_address_t page_address = {0};
  uint8_t msb = {0};
  uint8_t lsb = {0};
  int32_t ret = {0};
  uint8_t i = {0};
...
{
  lsm6dsr_counter_bdr_reg1_t counter_bdr_reg1 = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_device_id_get(const stmdev_ctx_t *ctx, uint8_t *val)
{
  ...
  ret = lsm6dsr_read_reg(ctx, LSM6DSR_WHO_AM_I, val, 1);
                                      ...
                                      lsm6dsr_hp_slope_xl_en_t *val)
{
  ...
  else
  {
    ...
    {
      case 0x00:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_4 : LSM6DSR_SLOPE_ODR_DIV_4;
        break;

      case 0x01:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_10 : LSM6DSR_HP_ODR_DIV_10;
        break;

      case 0x02:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_20 : LSM6DSR_HP_ODR_DIV_20;
        break;

      case 0x03:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_45 : LSM6DSR_HP_ODR_DIV_45;
        break;

      case 0x04:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_100 : LSM6DSR_HP_ODR_DIV_100;
        break;

      case 0x05:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_200 : LSM6DSR_HP_ODR_DIV_200;
        break;

      case 0x06:
        *val = is_low_pass == 1u ? LSM6DSR_LP_ODR_DIV_400 : LSM6DSR_HP_ODR_DIV_400;
        break;
...
int32_t lsm6dsr_aux_xl_flag_data_ready_get(const stmdev_ctx_t *ctx,
                                           uint8_t *val)
{
  lsm6dsr_status_spiaux_t status_spiaux = {0};
  int32_t ret = {0};
                                     ...
                                     lsm6dsr_st_xl_ois_t val)
{
  ...
  if (ret == 0)
  {
    int_ois.st_xl_ois = (uint8_t)(val & 0x03);
...
{
  lsm6dsr_int_ois_t int_ois = {0};
  lsm6dsr_ctrl1_ois_t ctrl1_ois = {0};
  int32_t ret = {0};
                                 ...
                                 lsm6dsr_sim_ois_t val)
{
  ...
  if (ret == 0)
  {
    ctrl1_ois.sim_ois = (uint8_t)(val & 0x01);
    ret = lsm6dsr_write_reg(ctx, LSM6DSR_CTRL1_OIS,
                            (uint8_t *)&ctrl1_ois, 1);
  }
...
{
  lsm6dsr_ctrl2_ois_t ctrl2_ois = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_ctrl3_ois_t ctrl3_ois = {0};
  int32_t ret = {0};
                                      ...
                                      lsm6dsr_fs_xl_ois_t val)
{
  ...
  if (ret == 0)
  {
    ctrl3_ois.fs_xl_ois = (uint8_t)(val & 0x03);
    ret = lsm6dsr_write_reg(ctx, LSM6DSR_CTRL3_OIS,
                            (uint8_t *)&ctrl3_ois, 1);
  }
...
{
  lsm6dsr_wake_up_dur_t wake_up_dur = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_wake_up_ths_t wake_up_ths = {0};
  int32_t ret = {0};
                                         ...
                                         lsm6dsr_sleep_status_on_int_t val)
{
  ...
  if (ret == 0)
  {
    tap_cfg0. sleep_status_on_int = (uint8_t)(val & 0x01);
    ret = lsm6dsr_write_reg(ctx, LSM6DSR_TAP_CFG0,
                            (uint8_t *)&tap_cfg0, 1);
  }
                             ...
                             lsm6dsr_inact_en_t val)
{
  ...
  if (ret == 0)
  {
    tap_cfg2.inact_en = (uint8_t)(val & 0x03);
    ret = lsm6dsr_write_reg(ctx, LSM6DSR_TAP_CFG2,
                            (uint8_t *)&tap_cfg2, 1);
  }
...
{
  lsm6dsr_tap_ths_6d_t tap_ths_6d = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_4d_mode_set(const stmdev_ctx_t *ctx, uint8_t val)
{
  ...
  if (ret == 0)
  {
    tap_ths_6d.d4d_en = (uint8_t)(val & 0x01);
    ret = lsm6dsr_write_reg(ctx, LSM6DSR_TAP_THS_6D,
                            (uint8_t *)&tap_ths_6d, 1);
  }
...
{
  lsm6dsr_free_fall_t free_fall = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_fifo_ctrl1_t fifo_ctrl1 = {0};
  lsm6dsr_fifo_ctrl2_t fifo_ctrl2 = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_compression_algo_init_set(const stmdev_ctx_t *ctx,
                                          uint8_t val)
{
  lsm6dsr_emb_func_init_b_t emb_func_init_b = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_fifo_ctrl2_t fifo_ctrl2 = {0};
  lsm6dsr_emb_func_en_b_t emb_func_en_b = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_fifo_ctrl3_t fifo_ctrl3 = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_batch_counter_threshold_set(const stmdev_ctx_t *ctx,
                                            uint16_t val)
{
  lsm6dsr_counter_bdr_reg2_t counter_bdr_reg1 = {0};
  lsm6dsr_counter_bdr_reg2_t counter_bdr_reg2 = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_fifo_data_out_tag_t fifo_data_out_tag = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_emb_func_fifo_cfg_t emb_func_fifo_cfg = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_sh_batch_slave_0_set(const stmdev_ctx_t *ctx,
                                     uint8_t val)
{
  lsm6dsr_slv0_config_t slv0_config = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_sh_batch_slave_1_set(const stmdev_ctx_t *ctx,
                                     uint8_t val)
{
  lsm6dsr_slv1_config_t slv1_config = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_sh_batch_slave_2_set(const stmdev_ctx_t *ctx,
                                     uint8_t val)
{
  lsm6dsr_slv2_config_t slv2_config = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_sh_batch_slave_3_set(const stmdev_ctx_t *ctx,
                                     uint8_t val)
{
  lsm6dsr_slv3_config_t slv3_config = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_emb_func_en_a_t emb_func_en_a = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_adv_pedo_t adv_pedo = {0};
  lsm6dsr_emb_func_en_b_t emb_func_en_b = {0};
  lsm6dsr_pedo_cmd_reg_t pedo_cmd_reg = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_pedo_step_detect_get(const stmdev_ctx_t *ctx,
                                     uint8_t *val)
{
  lsm6dsr_emb_func_status_mainpage_t emb_func_status = {0};
  int32_t ret = {0};
...
int32_t lsm6dsr_motion_flag_data_ready_get(const stmdev_ctx_t *ctx,
                                           uint8_t *val)
{
  lsm6dsr_emb_func_status_t emb_func_status = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_mag_cfg_a_t mag_cfg_a = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_mag_cfg_b_t mag_cfg_b = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_fsm_long_counter_clear_t fsm_long_counter_clear = {0};
  int32_t ret = {0};
                            ...
                            uint8_t *val)
{
  ...
  if (ret == 0)
  {
    ret = lsm6dsr_read_reg(ctx, LSM6DSR_FSM_OUTS1, val, 16);
  }
...
{
  lsm6dsr_emb_func_odr_cfg_b_t emb_func_odr_cfg_b = {0};
  int32_t ret = {0};
                                           ...
                                           uint8_t *val)
{
  ...
  ret = lsm6dsr_ln_pg_write_byte(ctx, LSM6DSR_FSM_PROGRAMS, val);
                                           ...
                                           uint8_t *val)
{
  ...
  ret = lsm6dsr_ln_pg_read_byte(ctx, LSM6DSR_FSM_PROGRAMS, val);
...
{
  lsm6dsr_s4s_tph_l_t s4s_tph_l = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_s4s_st_cmd_code_t s4s_st_cmd_code = {0};
  int32_t ret = {0};
...
{
  lsm6dsr_s4s_dt_reg_t s4s_dt_reg = {0};
  int32_t ret = {0};
